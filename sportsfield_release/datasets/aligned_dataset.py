...
from utils import warp, image_utils
...
class AlignedDataset(data.Dataset, abc.ABC):
    ...
    def load_template(self):
        ...
        self.template = util.np_img_to_torch_img(self.template)
    ...
    def get_homography_by_index(self, index):
        ...
        homography = util.to_torch(homography)
    ...
    def get_image_by_index(self, index):
        ...
        img = img[..., [2, 1, 0]]
        img = img / 255.0
        img = util.to_torch(img).permute(2, 0, 1)
        if self.opt.need_single_image_normalization:
