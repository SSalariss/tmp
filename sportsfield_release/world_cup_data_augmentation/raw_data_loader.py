...
class RawDataloader():
    ...
    def load_template(self):
        ...
        self.template_torch = util.to_torch(imageio.imread(self.template_path, pilmode='RGB') / 255.0).permute(2, 0, 1)
    ...
    def get_homography_by_path(self, homo_path):
        def get_frame_space_scaling_homography():
            src_4pts = util.to_torch(util.FULL_CANON4PTS_NP())
            dest_4pts = util.to_torch(np.array([[0, 0], [0, self.frame_h], [self.frame_w, self.frame_h], [self.frame_w, 0]], dtype=np.float32))
            scaling_transformation = util.get_perspective_transform(src_4pts[None], dest_4pts[None])
            scaling_transformation = util.to_numpy(scaling_transformation[0])
            return scaling_transformation

        def get_template_space_scaling_homography():
            src_4pts = util.to_torch(np.array([[0, 0], [0, self.template_h], [self.template_w, self.template_h], [self.template_w, 0]], dtype=np.float32))
            dest_4pts = util.to_torch(util.FULL_CANON4PTS_NP())
            scaling_transformation = util.get_perspective_transform(src_4pts[None], dest_4pts[None])
            scaling_transformation = util.to_numpy(scaling_transformation[0])
            return scaling_transformation
    ...
    def get_warped_tmp_by_id(self, data_id):
        ...
        warped_tmp = warp.warp_image(self.template_torch, util.to_torch(homo_mat), out_shape=(self.frame_h, self.frame_w))
        warped_tmp = util.torch_img_to_np_img(warped_tmp[0])
        return warped_tmp
